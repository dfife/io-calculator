...
double rec_TLA_dxHIIdlna(REC_COSMOPARAMS *cosmo, double xe, double xHII, double nH, double H, double TM, double TR, double Fudge) {
  ...
  RLya      *= cosmo->io_line_escape_factor;
  ...
  four_betaB *= cosmo->io_inverse_upward_rates_factor;
...
double rec_swift_hyrec_dxHIIdlna(HYREC_DATA *data, double xe, double xHII, double nH, double H, double TM, double TR, double z){
  ...
  Beta[0] *= cosmo->io_inverse_upward_rates_factor;
  Beta[1] *= cosmo->io_inverse_upward_rates_factor;
...
void populate_Diffusion(double *Aup, double *Adn, double *A2p_up, double *A2p_dn,
                        double TM, double Eb_tab[NVIRT], double A1s_tab[NVIRT], double io_lya_diffusion_factor) {
  ...
  *A2p_up *= io_lya_diffusion_factor;
  *A2p_dn *= io_lya_diffusion_factor;
  for (b = 0; b < NVIRT; b++) {
    Aup[b] *= io_lya_diffusion_factor;
    Adn[b] *= io_lya_diffusion_factor;
  }
...
void populateTS_2photon(double Trr[2][2], double *Trv[2], double *Tvr[2], double *Tvv[3],
                        ...
                        double io_inverse_upward_rates_factor, double io_line_escape_factor, double io_lya_diffusion_factor,
  ...
  RLya *= io_line_escape_factor;

  interpolate_rates(Alpha, DAlpha, Beta, &R2p2s, TR, TM / TR, atomic, fsR, meR, error, error_message);
  Beta[0] *= io_inverse_upward_rates_factor;
  Beta[1] *= io_inverse_upward_rates_factor;
  if (*error == 1) {
 ...
 /****** 2s row and column ******/
  ...
  populate_Diffusion(Aup, Adn, &A2p_up, &A2p_dn, TM, atomic->Eb_tab, atomic->A1s_tab, io_lya_diffusion_factor);
...
double rec_HMLA_2photon_dxHIIdlna(HYREC_DATA *data, double xe, double xHII, double nH, double H, double TM, double TR,
  ...
  populateTS_2photon(Trr, Trv, Tvr, Tvv, sr, sv, Dtau, xe, xHII, TM, TR, nH, H, atomic,
                     Dfplus, Dfplus_Ly, Alpha, DAlpha, Beta, fsR, meR, cosmo->inj_params->exclya,
                     cosmo->io_inverse_upward_rates_factor, cosmo->io_line_escape_factor, cosmo->io_lya_diffusion_factor,

...
void hyrec_init() {
  ...
  hyrec_allocate(&rec_data, zmax, zmin, MODEL);
  chdir(buffer);
  free(buffer);
}
...
*************************************************************************/
double hyrec_integrator(double deriv, double deriv_prev[2], int model) {
  double result;
...
void rec_get_xe_next1_He(HYREC_DATA *data, double z_in, double *xHeII, double dxHeIIdlna_prev[2],
  ...
  else *xHeII += DLNA * hyrec_integrator(dxHeIIdlna, dxHeIIdlna_prev, data->runtime_model);
...
void get_rec_next2_HHe(HYREC_DATA *data, int model, double z_in, long iz, double *xH1, double *xHeII,
  ...
  dxHeIIdlna  = rec_helium_dxHeIIdlna(data, z_in, *xH1, *xHeII, H);
  *xHeII     += DLNA * hyrec_integrator(dxHeIIdlna, dxHeIIdlna_prev, model);
...
void rec_get_xe_next1_H(HYREC_DATA *data, int model, double z_in, long iz, double xe_in, double Tm_in,
  ...
  if (data->quasi_eq == 1) {
    ...
    *xe_out = xe_in + DLNA * hyrec_integrator(dxedlna, dxedlna_prev, model);
  }
...
void rec_get_xe_next2_HTm(HYREC_DATA *data, int model, double z_in, long iz, double dxedlna_prev[2],
  ...
  if (data->Tm_evolve_implicit == 1){
    ...
    data->xe_output[iz] = xe_in + DLNA *hyrec_integrator(dxedlna, dxedlna_prev, model);
    data->Tm_output[iz] = Tm_implicit(z_out, data->xe_output[iz], Tm_in, cosmo, dEdtdV, H_next, DLNA);
  }
  else {
    data->xe_output[iz] = xe_in + DLNA *hyrec_integrator(dxedlna, dxedlna_prev, model);
    data->Tm_output[iz] = Tm_in + DLNA *hyrec_integrator(dTmdlna, dTmdlna_prev, model);
  }
...
char* rec_build_history(HYREC_DATA *data, int model, double *hubble_array){
  ...
  double *xH_output = data->xH_output, *xHe_output = data->xHe_output;
  ...
  ********/
  ...
  for(; z > zend; iz++) {
    ...
    xH_output[iz] = data->xe_output[iz];
    xHe_output[iz] = 0.;
    z  = (1.+zstart)*exp(-DLNA*iz) - 1.;
    z_out  = (1.+zstart)*exp(-DLNA*(iz+1)) - 1.;
    if (z < zend) z=0.;
...
void hyrec_allocate(HYREC_DATA *data, double zmax, double zmin, int runtime_model) {
  double DLNA;
  if (runtime_model == SWIFT) DLNA = DLNA_SWIFT;
  else DLNA = DLNA_HYREC;

  data->error = 0;
  data->runtime_model = runtime_model;
  ...
  if (runtime_model == FULL)  allocate_radiation(data->rad, (long int) (log(10.)/DLNA), &data->error, data->error_message);
  ...
  data->xH_output = create_1D_array(data->Nz, &data->error, data->error_message);
  data->xHe_output = create_1D_array(data->Nz, &data->error, data->error_message);
}
...
void hyrec_free(HYREC_DATA *data) {
  ...
  free(data->Tm_output);
  free(data->xH_output);
  free(data->xHe_output);
  free(data->error_message);
  if (data->runtime_model == FULL) free_radiation(data->rad);
  free(data->rad);
  free_fit(data->fit);
  free(data->fit);
}
...
double hyrec_xHII(double z, HYREC_DATA *data) {
  if (z > data->zmax) return data->xH_output[0];
  if (z < data->zmin) {
    fprintf(stderr, "\033[1m\033[31m error\033[22;30m in hyrec_xHII: requesting x_HII at z = %f ", z);
    fprintf(stderr, "lower than zmin\n");
    exit(1);
  }
  double DLNA = data->cosmo->dlna;
  return rec_interp1d(-log(1.+data->zmax), DLNA, data->xH_output, data->Nz, -log(1.+z), &data->error, data->error_message);
}

double hyrec_xHeII(double z, HYREC_DATA *data) {
  if (z > data->zmax) return data->xHe_output[0];
  if (z < data->zmin) {
    fprintf(stderr, "\033[1m\033[31m error\033[22;30m in hyrec_xHeII: requesting x_HeII at z = %f ", z);
    fprintf(stderr, "lower than zmin\n");
    exit(1);
  }
  double DLNA = data->cosmo->dlna;
  return rec_interp1d(-log(1.+data->zmax), DLNA, data->xHe_output, data->Nz, -log(1.+z), &data->error, data->error_message);
}
